        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('admin.kamar.update', $room) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="nama_kamar" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Kamar</label>
                            <input type="text" name="nama_kamar" id="nama_kamar" value="{{ old('nama_kamar', $room->nama_kamar) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <select name="tipe" id="tipe" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white @error('tipe') border-red-500 @enderror">
                                ...
                                @foreach(['Standard', 'Deluxe', 'Suite', 'Executive'] as $tipe)
                                    <option value="{{ $tipe }}" {{ old('tipe', $room->tipe) == $tipe ? 'selected' : '' }}>{{ $tipe }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4 grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" min="0" name="harga_per_malam" id="harga_per_malam" value="{{ old('harga_per_malam', $room->harga_per_malam) }}"
                            ...
                            <div>
                                ...
                                <input type="number" min="1" name="kapasitas" id="kapasitas" value="{{ old('kapasitas', $room->kapasitas) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" min="0" name="stok" id="stok" value="{{ old('stok', $room->stok) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="4"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $room->deskripsi) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            @if($room->gambar)
                                <img src="{{ asset('storage/' . $room->gambar) }}" class="w-32 h-24 object-cover rounded mb-2">
                            @endif
                            <input type="file" name="gambar" id="gambar" accept="image/*"
                                class="mt-1 block w-full text-gray-700 dark:text-gray-200 @error('gambar') border-red-500 @enderror">
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Kosongkan jika tidak ingin mengubah foto.</p>
